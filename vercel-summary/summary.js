...
async function fetchAllResults() {
    ...
    try {
        const resp = await fetch(noCacheUrl(DATA_URL), { cache: 'no-store' });
        if (!resp.ok) {
        ...
        setHtml(elements.tableBody, '<tr><td colspan="12" class="px-4 py-8 text-center text-red-600">加载数据失败，请检查 results.json 是否存在且可访问</td></tr>');
        if (elements.cardsWrap) {
            setHtml(elements.cardsWrap, '<div class="px-4 py-8 text-center text-red-600">加载数据失败，请检查 results.json 是否存在且可访问</div>');
        }
    } finally {
